...
A word of warning, of course— the process is somewhat involved, and takes some motivation to push through. This is a great example of "you can do things cheaply— if you have the patience and know-how".
...
With a full HD display, 128 gigs of storage, and 8-16[^swap] gigs of RAM, this phone is **better** than the Macbook Air I used for 10 years— and incredibly cheaper.

[^swap]: The native "RAM boost" setting adds up to 8GB of swap memory.
...
## 2.2 Peripherals
![](images/Targus.jpg){width=580 fig-align="center"}
...
Settings -> `System` -> `Performance` -> `RAM boost` -> `8gb`
:::
...
pkg install termux-api x11-repo termux-x11-nightly pulseaudio xfce4 virglrenderer-android
```
...
3. Paste this script ([original source](https://github.com/LinuxDroidMaster/Termux-Desktops/blob/main/scripts/termux_native/startxfce4_termux.sh))[^sorc] and save the file (tap and hold and select paste, then `ctrl+s ctrl+x` to save and exit.)

[^sorc]: Normally I'd just say "copy from this link", but there was a Termux guide once that did this and had a broken link!
...
# Start GPU backend (remove this if you don't want hardware acceleration)
pkill -f virgl
...
You'll notice that the Android keyboard hotkeys (like cmd+tab) still apply; I just got used to avoiding them, but it's also possible to disable them via adb.
...
By including the USB-C adapter, it means I also have a **portable adapter** whenever I need it to connect a regular USB to the phone. (Which is much more useful than the extra storage, lol)
...
**micro** is a *modern* lightweight text editor— it serves the same role as `nano`, but has nicer features like pretty code highlighting. Alternatively you can use `nano -/ <file>` to get the same modern keybinds in nano.
...
![](images/cpufetch.png){width=800 fig-align="center"}
...
echo "alias debian='proot-distro login debian --shared-tmp'" >> ~/.bashrc && exec bash
debian
ln -s /data/data/com.termux/files/home termux
echo "export DISPLAY=:0" >> ~/.bashrc && exec bash
apt update
```
...
Proot debian can **run anything already installed** on Termux, but you can also use `apt` to install Debian packages that would otherwise be unavailable. I've had [great success](/flipphone-r/flipphone-r.qmd) running the R language this way, for example.

You can make a launcher on the Xfce desktop that opens a terminal window running proot debian: right-click on the desktop, `Create Launcher`, enable `Run in terminal`, and set the command to `proot-distro login debian`[^aww]. You can get an icon for the launcher [here](https://www.debian.org/logos/) (use the SVG one for background transparency).

[^aww]: Apparently launcher commands can't use bash aliases (like the `debian` one we just created), probably because they don't run .bashrc before executing.

---

It's unwise, but you can also use this to easily emulate **x86 architecture**, EXTREMELY SLOWLY.

```bash
pkg install qemu-user-x86-64
proot-distro install ubuntu --architecture x86_64
proot-distro login ubuntu --shared-tmp
echo "export DISPLAY=:0" >> ~/.bashrc && exec bash
```

I was able to get a full graphical x86 librewriter (open-source microsoft word) instance running with this (`apt install libreoffice && lowriter`). But of course— x86 emulation is extremely slow, and it wasn't very practical. Would not recommend; stick to native proot arches.
...
**Type with voice**:  
Usage: say "say <text> over"; it will inject <text> as keyboard text into whatever window you're focused on. `termux-speech-to-text` likes to cut off the final word so you have to end with a dummy phrase like "over".

```bash
while true; do
txt="$(termux-speech-to-text)"
txt="${txt,,}" #lowercase
cmd="${txt%% *}" #get first word
txt="${txt#* }" #remove first word
test "$cmd" = "say" && xdotool type "$txt"
done
```

Termux-API also offers some pretty powerful tools for using your phone as a sensor, with commands like `termux-location` (GPS), `termux-torch` (flashlight), and `termux-sensor` (gyroscope et al.).
...
![This was a live video view from the phone camera!](images/AR.png){width=800 fig-align="center"}

With so much control over the phone, one of the things I wanted to try was **streaming the phone's camera onto the desktop**. This would allow you to have a (very shoddy!) monitor-glasses-like experience if you attached the phone to your face— but since even the lowest-priced monitor glasses are $300+, it'd be a fun gimmick! A simple cardboard box with a fresnel lens should work to get the phone on your head (and focused+magnified), but I haven't gotten around to making one yet.

So I hacked together a way to put the camera on the desktop. This has some **rough edges**— it's certainly not in a complete state— but it does work, and it's pretty fun to mess around with.

## 6.1 Setup

First, install the [RemoteCam](https://github.com/Ruddle/RemoteCam) apk. This will stream the camera to a local IP address (I can't figure out how to point it to localhost ☹), which we can pick up from the desktop.
...
:::{.callout-note title="~/cameramirror.sh" collapse=true}
```bash
#!/bin/bash
...
1. Start the RemoteCam app.

2. Select a camera (the wide-angle one works well), resolution[^resol] (I recommend 1280x720), and change the quality to "10". 

[^resol]: Higher resolutions/quality will increase the latency of the stream. Below 720p, I get less than a quarter-second of delay, which is pretty passable for moving around.

3. Turn off preview and turn on the stream. Copy the stream's IP/URL (something like `192.1.../cam.mjpeg`).

4. Open up a terminal window in Xfce, and run the script with that IP in an Xfce terminal:

```bash
. ~/cameramirror.sh <COPIED_URL>
```

5. If all goes well, the script should open the stream and project it onto a background window. Now you can continue using the desktop as normal, and you'll be able to see the phone camera output behind everything.

6. To close the stream, enter ^C into the original terminal window, and then close the window.

---

:::{.callout-caution}
This is mainly a proof of concept, so it hasn't received much testing or polish. The closing behavior in particular is somewhat buggy— when in doubt, a full phone restart will fix anything amiss.
:::

It may be worth messing with window opacity in Xfce's `Window Manager Tweaks` menu to make it easier to see the camera behind windows.
