...
I WANT AN ACTUAL OVERVIEW OF GGUF STRUCTURE

this should go play-by-play integrating structure and how to use— they can read it while it's loading!


(this is automatically loaded when you run `library(ggufr)`; I'm putting it here for ease of reading)


# setup
download_model and model = read_ggufr()
this requires 30gb of free space
GGUF is saved in r directory (delete with delete_model), weights are in tempdir and deleted when r session ends
...
while that's running, continue reading!

# read_gguf
binary file and raw reading in R
structure: a bunch of metadata, then the raw tensors
quantizing and unpacking/caching tensors

# tokenization
let's make a prompt: who is Hadley Wickham?
before feeding this into the model, we have to tokenize it (I'm sure you're familiar with this already— if not, plenty of guides online)
many LLMs in 2026 use BPE based on a merge dictionary, which is very convenient to implement
[how bpe works w/ the gguf tokens+merges]
now we take our token ids and use them to look up in the embedding matrix (what do the embeds represent? info at different ~~dims, why?). this is what we feed into transformer to start

# inference
so what will we do?
[what's prefilling, then afterwards generation]
feed embedded state into transformer
pass that through 36 layers
this is prefill so slice off the end. now we have an end state matrix, and we use it to get the next token
"choose" token from logits after multiplying with the original embedding matrix; this makes sense— since we're multiplying element-wise, a vector will be highest value if it matches the embedding matrix

so what's the math on each layer?

#transformation
for the first piece of math, we do the entire matrix, this is called prefilling

afterwards for prediction we just do this with single vectors, which is much easier. uses the same math too
...
This one's been a long time in the making. It's a bit of a personal endeavor— not the most novel thing, but very gratifying. AI can be scary, but being able to "make one" yourself really demystifies it!

Anyway, this article is about a little R package[^isit] I wrote to parse GGUF files and run inference (interactive chatbot) with a specific LLM. It all runs **natively in base R**— pretty slowly, of course, but this makes it easy to track, visualize, and understand how the math works. All code was made by hand <3

[^isit]: I'm not sure it really counts as a package— just a set of mostly undocumented functions you can use.

This writeup will be a bit more casual than they usually are, since this topic has been covered to death— I don't have as much reason to formally document it.
...
Here's a **blueprint** I made to show the exact byte format of a GGUF, with examples from the SmolLM3 GGUF we'll be using:

:::{.callout-note title="GGUF raw format" collapse=true}
```
#Header
(47 47 55 46) Magic Number
(uint32) GGUF Version
...
And here's an example of the parsed header, metadata, and tensor data from SmolLM3. This is a neatened view of what you'll get by running `view(model)`, since `read_gguf` stores all this info in a single R object.
...
:::{.callout-note title="Tensor Info" collapse=true}
```{r echo=F}
...
## 2.3 read_gguf
Everything I discussed in this section is performed by one function: `read_gguf`. Here it is for your reading convenience or curiosity:

:::{.callout-tip title="read_gguf.R" collapse=true}
```{=html}
<iframe frameborder="0" scrolling="no" style="width:100%; height:7159px;" allow="clipboard-write" src="https://emgithub.com/iframe.html?target=https%3A%2F%2Fgithub.com%2Fgbkorr%2Fggufr%2Fblob%2Fmaster%2FR%2Fread_gguf.R&style=vs&type=code&showBorder=on&showLineNumbers=on&showFullPath=on"></iframe>
```
:::



:::{.callout-tip title="tokenizer.R" collapse=true}
```{=html}
<iframe frameborder="0" scrolling="no" style="width:100%; height:4261px;" allow="clipboard-write" src="https://emgithub.com/iframe.html?target=https%3A%2F%2Fgithub.com%2Fgbkorr%2Fggufr%2Fblob%2Fmaster%2FR%2Ftokenizer.R&style=vs&type=code&showBorder=on&showLineNumbers=on&showFullPath=on"></iframe>
```
:::

:::{.callout-tip title="inferencer.R" collapse=true}
```{=html}
<iframe frameborder="0" scrolling="no" style="width:100%; height:2329px;" allow="clipboard-write" src="https://emgithub.com/iframe.html?target=https%3A%2F%2Fgithub.com%2Fgbkorr%2Fggufr%2Fblob%2Fmaster%2FR%2Finferencer.R&style=vs&type=code&showBorder=on&showLineNumbers=on&showFullPath=on"></iframe>
```
:::

:::{.callout-tip title="transformer.R" collapse=true}
```{=html}
<iframe frameborder="0" scrolling="no" style="width:100%; height:3904px;" allow="clipboard-write" src="https://emgithub.com/iframe.html?target=https%3A%2F%2Fgithub.com%2Fgbkorr%2Fggufr%2Fblob%2Fmaster%2FR%2Ftransformer.R&style=vs&type=code&showBorder=on&showLineNumbers=on&showFullPath=on"></iframe>
```
:::
...
library(ggufr)
```
Then run this function to download the LLM. It's about 3GB, and it'll be saved in your R directory; run `delete_model()` if you want to get rid of it.
```
download_model()
```

# 1.2 Loading the Model
To run inference, we first have to unpack the GGUF so that R can work with it. The GGUF is a raw binary file with its own special format, so I had to write a big function to read it properly and extract its data.

:::{.callout-warning}
Warning: fully unpacking the weights (which are required to run inference) will take up around 25GB. Make sure your computer has some free space!
These files will be deleted when you close R, since they're stored in `tempdir()`.
:::

Run this, which will take a while (up to 30 minutes):
```
model = read_gguf()
```
...
If you have a different GGUF file you'd like to look at, this package can't run it but can 

# 1.2.1 Loading Details
In the background, this function parses the GGUF (using custom byte-reading code— R is quite bad at handling raw binary...) and uses parsed info to extract the model's tensors. It saves these numbers— 3 billion weights in total!— to `tempdir()`, with an 
...
I took a unique approach to learning for thsi rpoject

Resources for understanding LLM architecture on a [engineering] level are quite [spread out, heterogenous] due to the fast-paced nature of AI development in the past few years. Many of the best guides are from 2023[^egguide] and 2024, where it's hard to tell what is and isn't outdated.
...
I ended up structuring this project **almost like a class**: I would ask Gemini to describe a part of the LLM implementation, and then implement it entirely on my own, occasionally asking for clarification and advice—but not code!—when needed.



This lecture -> try it out -> ask clarif was quite similar to tehe structure of classes, and worked tremendously well for me— because I was trying things out, I was building the important experience and understanding myself, and my own testing ironed out any innacuracies in the "teacher's" teaching.

focus on: implementing yourself is where ypou actually get a good grasp of the implementation



# the learning process (give this a good, article-like section header

learning
executing
debugging (gave up, impressed by gemini finding **implementation-level typos**, which requires a pretty deep understanding of how the code works and how that relates to the thing I'm trying to accomplish
(I've spent a lot of time debugging and know how to do it effectively— but when I know it's , this just seems to be far more effective. I'm not sure if I should adopt it going forward; maybe only when it matters)


**never any code written by AI**, but all knowledge was from AI, and it was used to debug
