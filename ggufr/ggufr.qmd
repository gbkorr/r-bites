...
:::{.callout-note title="read_gguf.R" collapse=true style="max-height: 600px; overflow-y: auto;"}
```{=html}
...
:::{.callout-note title="tokenizer.R" collapse=true style="max-height: 600px; overflow-y: auto;"}
```{=html}
...
:::{.callout-note title="inferencer.R" collapse=true style="max-height: 600px; overflow-y: auto;"}
```{=html}
...
:::{.callout-note title="transformer.R" collapse=true style="max-height: 600px; overflow-y: auto;"}
```{=html}
...
what are tokens
regex chunking
merge rules— this way, you'll always end up with tokenss (instead of needing to rely on the old <unk>)
deterministic! that's important, probably
actually merging is done with the raw bytes, which is convenient because of how it's stored

I actually save the raw bytes separately in the model object to facilitate this, but you could just as easily read from the GGUF
