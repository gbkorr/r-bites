...
Morstdio has a **modular command system**. If you type `":s "` (delete word), for example, it will remove the characters `":s "` and execute the "delete word" command. If you type something that isn't a command, like `":y "`, it will remain as plaintext.
